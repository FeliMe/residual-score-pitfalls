...
from utils import get_training_timings
...
class Trainer:
    ...
    def init_train_ds(self, train_files):
        ds = TrainDataset(files=train_files,
                          ...
                          load_to_ram=self.config.load_to_ram)
    ...
    def init_val_ds(self, val_files):
        ds = TrainDataset(files=val_files,
                          img_size=self.config.inp_size[0],
                          slice_range=self.config.slice_range,
                          load_to_ram=self.config.load_to_ram)
    ...
    def train(self, num_steps, train_files, val_files=None):
        ...
        while True:  # Stopping is handled by num_steps
            for x in trainloader:
                ...
                # Log if necessary
                if i_step % self.config.log_inverval == 0:
                    # Log timings
                    ...
                    print(f"Step [{i_step}|{num_steps}] - " \
                          ...
                          f"Time per step: {time_per_step} - ", end='')
                ...
                if i_step % self.config.val_inverval == 0 and valloader is not None:
                    val_loss = self.validate(valloader)
                    if val_loss < best_val_loss:
                        best_val_loss = val_loss
                        self.save_ckpt(f"best.pt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_ckpt", type=str, default=None,
                        help=f"in the form <run_name>/<last or best>.pt")
    ...
    parser.add_argument("--val_inverval", type=int, default=200)
